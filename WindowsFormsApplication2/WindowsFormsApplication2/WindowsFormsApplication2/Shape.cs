...
namespace WindowsFormsApplication2
{
    ...
    class Shape
    {
        ...
        {
            g.DrawRectangle(new Pen(Color.Red), topLeft.X - 10, topLeft.Y - 10, width + 20, height + 20);
        }
        ...
        public void DRAW(Graphics g, Color C)
        {
            ...
            if (shapetype == TYPE.rect)
            {
               // g.DrawRectangle(P, topLeft.X, topLeft.Y, width, height);
                ...
                else g.FillPolygon(new SolidBrush(Color.Red), poly);
            }
            else if (shapetype == TYPE.ellipse)
                if(stype==TYPE2.DRAW)
                g.DrawEllipse(P, topLeft.X, topLeft.Y, width, height);
                else g.FillEllipse(new SolidBrush(Color.Red),topLeft.X, topLeft.Y, width, height);
        }
        Point topLeft;
